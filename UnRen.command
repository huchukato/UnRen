...
# UnRen for Mac v 0.9.0 by huchukato - https://f95zone.to/members/huchukato.11155677/
# Based on UnRen for Mac v 0.8.2 by goobdoob - https://f95zone.to/members/goobdoob.334840/
...
# unrpyc v2.x - https://github.com/CensoredUsername/unrpyc (Python 3 version)

# Change Log:
#
# v0.9.0:
# Major upgrade: Updated to use Python 3 instead of Python 2
# Updated to unrpyc v2.x (Python 3 version)
# Updated rpatool for Python 3 compatibility
# Added optional Ren'Py SDK path configuration for decompiler
# Added warning messages for decompiler requirements
#
# v0.8.2:
...
# Configuration
    quicksavekey="K_F5"
    quickloadkey="K_F9"
    # Opzionale: percorso dell'SDK Ren'Py per il decompilatore
    # Se non impostato, il decompiler userà i moduli Ren'Py dal sistema se disponibili
    renpy_sdk_path=""
...
function init
{
    ...
    echo "UnRen for Mac and Linux v$version for Python 3"
    echo "by huchukato @ f95zone.to"
    echo '___________________________________________ '    
    echo ''
    echo "UnRen.sh is based on the original version created by goobdoob @ f95zone.to,"
    echo "and the Windows version of UnRen.bat by sam @ f95zone.to"
    echo ''

    if command -v $python &>/dev/null; then
        echo "Python 3 is installed, detected"
    else
        echo "$python is not installed, or not added to your PATH. Please install/reinstall it from python.org"
        echo "This version requires Python 3"
        exit
    fi
...
function extract
{
    ...
    if [ -e ${files[0]} ]; then
        ...
        for f in *.rpa; do
            ...
            $python "$rpatool" -x -v "$f"
            echo "Extraction of $f completed"
        done
        echo "All RPA packages extracted to: $(pwd)"
    else
...
function decompile
{
    ...
    if [ -z "$renpy_sdk_path" ]; then
        echo "Ren'Py SDK path not configured in the Configuration section."
        echo "Please enter the path to your Ren'Py SDK (or press Enter to skip):"
        read user_sdk_path
        if [ -n "$user_sdk_path" ]; then
            export PYTHONPATH="$user_sdk_path:$PYTHONPATH"
            echo "Using Ren'Py SDK from: $user_sdk_path"
        else
            echo "WARNING: Decompiler requires Ren'Py SDK to be installed and in your PATH"
            echo "If you get import errors, set renpy_sdk_path in the Configuration section"
        fi
    else
        echo "Using Ren'Py SDK from: $renpy_sdk_path"
    fi
    pushd "$game" > /dev/null
    $python "$unrpyc" "."
    popd > /dev/null
